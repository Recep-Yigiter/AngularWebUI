...
import { CreateTeklifModel } from '../core/models/create-teklif-model';

@Component({
  selector: 'app-create-verilen-teklif',
  templateUrl: './create-verilen-teklif.component.html',
  styleUrls: ['./create-verilen-teklif.component.scss'],
  providers: [CurrencyPipe, DatePipe],
})
export class CreateVerilenTeklifComponent {
  ...
  kaydet() {
    ...
    if (this.getAllRowData().length > 0) {
      this.TeklifService.create(createModel, () => {
        this.router.navigate(['/pages/satis/detail-verilen-teklif'], { state: createModel })
      }, errorMessage => { })
  ...
  vazgec() {
    this.router.navigate(['/pages/satis/verilen-teklif'])
  }
  ...
  changed(event) {
    ...
    this.defaultAciklama = this.selectedTeklifTuru?.seri + "-" + this.frm.value.belgeNo + " no lu Verilen Teklif"
