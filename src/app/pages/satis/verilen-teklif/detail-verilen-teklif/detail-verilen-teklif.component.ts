...
export class DetailVerilenTeklifComponent implements OnInit {
  ...
  vazgec() {
    ...
    this.router.navigate(['/menu/satis/verilen-teklif'], { state: this.stateData })

  }
  async duzenle() {

    if (this.stateData?.id) {
      this.router.navigate(['/menu/satis/verilen-teklif/update'], { state: this.stateData })
    }
    else {
      this.teklif = (await this.TeklifService.getByHourId(this.stateData.hourId, () => { }))

      this.router.navigate(['/menu/satis/verilen-teklif/update'], { state: this.teklif })
    }
  }
