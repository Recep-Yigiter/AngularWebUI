...
import { SiparisHareketService } from '../core/services/siparis-hareket.service';
import { UpdateSiparisModel } from '../core/models/update-siparis-model';

@Component({
  selector: 'app-update-alinan-siparis',
  templateUrl: './update-alinan-siparis.component.html',
  styleUrls: ['./update-alinan-siparis.component.scss'],
  providers: [CurrencyPipe, DatePipe],
})
export class UpdateAlinanSiparisComponent implements OnInit {
  defaultAciklama: any;
  stateData: any;
  defaultKDV: any = 0;
  defaultOTV: any = 0;
  ...
  kaydet() {
    ...
    if (this.getAllRowData().length > 0) {
      this.SiparisService.update(createModel, () => {
        this.router.navigate(['/pages/satis/detail-alinan-siparis'], { state: createModel })
      }, errorMessage => { })
    } else {
  ...
  kaydet_yeni() {
    ...
    if (this.getAllRowData().length > 0) {
      this.SiparisService.update(createModel, () => {
        this.router.navigate(['/pages/satis/update-alinan-siparis'])
      }, errorMessage => { })
    } else {
      alert('Siparis e Satır Eklemelisiniz !')
    }
  ...
  vazgec() {
    ...
    this.router.navigate(['/pages/satis/alinan-siparis'], { state: this.stateData })
  }
