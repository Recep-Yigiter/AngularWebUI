...
@Component({
  selector: 'app-update-alis-fatura',
  templateUrl: './update-alis-fatura.component.html',
  styleUrls: ['./update-alis-fatura.component.scss'],
  providers: [ DatePipe],
})
export class UpdateAlisFaturaComponent implements OnInit {
  @Input() data: any;
  rowData: any[];
  ...
  Kaydet() {
    ...
    createModel.hareketTipi=this.data.hareketTipi;
    createModel.hareketAdi=this.data.hareketAdi;
    createModel.hareketId=this.data.hareketId;
    ...
    createModel.faturaHareketler.forEach((element) => {
      element.depoId = this.selectedDepo?.id? this.selectedDepo.id: this.data.depoId;
    });

    this.FaturaService.delete(this.data.id);
  ...
  async depoBazindaStokGuncelle(event) {
    ...
    const result = event.faturaHareketler.filter((f) => !maps.get(f.stokId));
  ...
  async getList(params: GridReadyEvent<any>) {
    ...
    this.rowData = this.data.faturaHareketler;
  }
  rowClick(event) {
  ...
  async dataControl() {
    ...
    this.data.depoId = this.data.faturaHareketler[0].depoId;
    this.data.depoAdi = this.data.faturaHareketler[0].depoAdi;
    this.data.depoKodu = this.data.faturaHareketler[0].depoKodu;

    this.data.satirSayisi = this.data.faturaHareketler.length;
    this.data.faturaHareketler.forEach((item, index) => {
    ...
    this.frmSatirTutari = this.data.faturaHareketler.reduce(
      (prev: any, next: any) => prev + next.satirTutar,
      0
    );
    this.frmIskontoTutari = this.data.faturaHareketler.reduce(
