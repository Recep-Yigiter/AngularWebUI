...
this.rowData=this.rowData.filter(c=>c.seri=="AF");
 this.rowData.sort((val1, val2)=> {return <any> new Date(val2.createdDate) - <any> new Date(val1.createdDate)})
    ...
    this.rowData.forEach((siparis) => {
      ...
      siparis.satirTutar = siparis.faturaHareketler.reduce((prev: any, next: any) => prev + next.satirTutar, 0)
      siparis.iskontoTutar =siparis.faturaHareketler.reduce((prev: any, next: any) => prev + next.iskontoTutar, 0)
      siparis.iskontoSonrasiTutar = siparis.faturaHareketler.reduce((prev: any, next: any) => prev + next.iskontoSonrasiTutar, 0)
      siparis.kdvTutar = siparis.faturaHareketler.reduce((prev: any, next: any) => prev + next.kdvTutar, 0);
