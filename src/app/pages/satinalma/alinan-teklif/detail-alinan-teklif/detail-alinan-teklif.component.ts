...
export class DetailAlinanTeklifComponent implements OnInit {
  ...
  async duzenle() {
    ...
    if (this.stateData?.id) {
      this.router.navigate(['/menu/satinalma/alinan-teklif/update'], { state: this.stateData })
    }
    else {
      this.teklif = (await this.TeklifService.getByHourId(this.stateData.hourId, () => { }))

      this.router.navigate(['/menu/satinalma/alinan-teklif/update'], { state: this.teklif })
    }
  }
