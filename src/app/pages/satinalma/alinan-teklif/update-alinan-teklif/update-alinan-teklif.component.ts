...
@Component({
  ...
  templateUrl: './update-alinan-teklif.component.html',
  styleUrls: ['./update-alinan-teklif.component.scss'],
  providers: [DatePipe],
})
export class UpdateAlinanTeklifComponent implements OnInit {
  @Input() data: any;
  rowData: any[];
  ...
  Kaydet() {
    ...
    createModel.teklifHareketler = this.getAllRowData();
     ...
     this.TeklifService.delete(this.data.id);
  ...
  async getList(params: GridReadyEvent<any>) {
    ...
    this.rowData = this.data.teklifHareketler;
  }
  getAllRowData() {
    let rowData = [];
  ...
  async dataControl() {
    ...
    this.data.satirSayisi = this.data.teklifHareketler.length;
    this.data.teklifHareketler.forEach((item, index) => {
    ...
    this.frmSatirTutari = this.data.teklifHareketler.reduce(
      (prev: any, next: any) => prev + next.satirTutar,
      0
    );
    this.frmIskontoTutari = this.data.teklifHareketler.reduce(
