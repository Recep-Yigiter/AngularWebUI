...
import { TeklifHareketService } from '../core/services/teklif-hareket.service';
...
export class UpdateAlinanTeklifComponent implements OnInit {
  ...
  kaydet_yeni() {
    ...
    if (this.getAllRowData().length > 0) {
      this.TeklifService.update(createModel, () => {
        this.router.navigate(['/pages/satin-alma/create-verilen-teklif'])
      }, errorMessage => { })
    } else {
      alert('Teklif e Satır Eklemelisiniz !')
    }
  ...
  vazgec() {
    ...
    this.router.navigate(['/pages/satin-alma/verilen-teklif'], { state: this.stateData })
  }
