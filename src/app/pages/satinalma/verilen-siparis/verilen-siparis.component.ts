...
@Component({
  selector: 'app-verilen-siparis',
  templateUrl: './verilen-siparis.component.html',
  styleUrls: ['./verilen-siparis.component.scss'],
  providers: [DatePipe],
})
export class VerilenSiparisComponent implements OnInit {
...
this.rowData=this.rowData.filter(c=>c.seri=="VS")
  ...
  onSelectionChanged() {
    ...
    const selectedRows = this.gridApi.getSelectedRows()[0];
    this.selectedSiparisHareket = selectedRows
    this.router.navigate(['/pages/satin-alma/detail-verilen-siparis'], { state: selectedRows })
  }
