...
@Component({
  selector: 'app-detail-verilen-siparis',
  templateUrl: './detail-verilen-siparis.component.html',
  styleUrls: ['./detail-verilen-siparis.component.scss'],
  providers: [ DatePipe],
})
export class DetailVerilenSiparisComponent implements OnInit {
  /**
   *
   */
  stateData: any;
  ...
  async duzenle() {
    ...
    if (this.stateData?.id) {
      this.router.navigate(['/pages/satin-alma/update-verilen-siparis'], { state: this.stateData })
    }
    else {
      this.siparis = (await this.SiparisService.getByHourId(this.stateData.hourId, () => { })).data

      this.router.navigate(['/pages/satin-alma/update-verilen-siparis'], { state: this.siparis })
    }
  }
