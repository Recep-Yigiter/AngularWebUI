...
export class DetailVerilenSiparisComponent implements OnInit {
  ...
  async duzenle() {
    ...
    if (this.stateData?.id) {
      this.router.navigate(['/menu/satinalma/verilen-siparis/update'], { state: this.stateData })
    }
    else {
      this.siparis = (await this.SiparisService.getByHourId(this.stateData.hourId, () => { }))

      this.router.navigate(['/menu/satinalma/verilen-siparis/update'], { state: this.siparis })
    }
  }
