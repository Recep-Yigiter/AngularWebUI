...
import { AlinanTeklifHareketModalComponent } from '../alinan-teklif-hareket-modal/alinan-teklif-hareket-modal.component';
...
export class AlinanTeklifModalComponent implements OnInit {
  ...
  async getList(params: GridReadyEvent<any>) {
    ...
    this.rowData = this.rowData.filter(c => c.seri == "AT" && c.cariId == this.selectedCari.id && c.durum == "Açık");
  ...
  ileri() {
    if (this.selectedRows.length > 0) {
      const modalRef = this.modalService.open(AlinanTeklifHareketModalComponent, { size: 'lg', backdrop: 'static' },);
