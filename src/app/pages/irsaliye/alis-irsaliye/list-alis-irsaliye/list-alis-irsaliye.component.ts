...
@Component({
  ...
  styleUrls: ['./list-alis-irsaliye.component.scss'],
  providers: [DatePipe],
})
export class ListAlisIrsaliyeComponent implements OnInit {
  rowData: any[];
  ...
  createModal() {
    const modalRef = this.NgbModal.open(CreateAlisIrsaliyeComponent, {
      size: 'xl',
      backdrop: 'static',
    });
  ...
  updateModal() {
    if (this.selectedRow) {
      const modalRef = this.NgbModal.open(UpdateAlisIrsaliyeComponent, {
        size: 'xl',
        backdrop: 'static',
      });
