...
@Component({
  selector: 'app-genel-virman-cari-select',
  template: `
...
export class GenelVirmanCariSelectComponent
  implements ICellEditorAngularComp, AfterViewInit
{
  ...
  selectDialogOpen() {
    const modalRef = this.modalService.open(CariSelectModalComponents, {
      size: 'xl',
      backdrop: 'static',
    });
    modalRef.componentInstance.confirmationBoxTitle = 'Arama : Bileşen';
    modalRef.result.then((cari) => {
      this.selectedItem = cari;
      ...
      this.params.data.cariId=this.selectedItem.id
